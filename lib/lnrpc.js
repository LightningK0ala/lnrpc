...
module.exports = async function createLnprc(config = {}) {
  ...
  try {
    ...
    // remove google annotations causing parse error
  ...
  try {
    const pkgDef = protoLoader.loadSync(protoDest, {
      // Preserve field names, otherwise they will be changed to camelCase
      keepCase: true
    })
    pkgObj = grpc.loadPackageDefinition(pkgDef)
  } catch (e) {
  ...
  const lnrpc = Object.create(null, {
    description: {value: pkgObj},
    lightning: {
      value:
        lightning || createLightning(pkgObj, server, credentials, config),
    },
    walletUnlocker: {
      value:
        walletUnlocker || createWalletUnlocker(pkgObj, server, credentials),
    },
  });
